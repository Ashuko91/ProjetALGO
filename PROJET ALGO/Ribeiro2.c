...
// Version recursive terminale : NE MARCHE PAS 
Liste FctBegaye1_aux(Liste l, Liste res){
    if (estVide(l)){
        return res;
    } else {
        if (premier(l) > 0){
            return FctBegaye1_aux(suite(l), ajoute(premier(l), res));
        } else {
            return FctBegaye1_aux(suite(l), res);
        }
    }
...
int main(int argc, char const *argv[]){
    ...
    printf("Liste l2 : ");
    affiche_iter(l2);
    printf("ZPUDT sur l1 (1 pour TRUE, 0 pour FALSE): %d\n", ZeroEnPositionUnOuDeuxOuTrois(l1));
    printf("Pluscourte sur l1 (1 pour TRUE, 0 pour FALSE): %d\n", Pluscourte(l1,l2));
    int k = 3;
    printf("NZAPK Version1 sur l1 pour k = %d : %d\n", k, NombreDeZeroAvantPositionK_1(l1,k));
    printf("NZAPK Version2 sur l1 pour k = %d : %d\n", k, NombreDeZeroAvantPositionK_2(l1,k));
    printf("NZAPK Version3 sur l1 pour k = %d : %d\n", k, NombreDeZeroAvantPositionK_3(l1,k)); // Cette version ne donne pas le bon resultat        
    printf("NZAPK Version4 sur l1 pour k = %d : %d\n", k, NombreDeZeroAvantPositionK_4(l1,k));        
    //printf("NZARPK : %d\n", NombreDeZeroApresRetroPositionK(l1,2)); // Cette fonction ne marche pas
    
    //printf("FctBegaye sur l1 : ");
    //affiche_rec(FctBegaye(l1)); // Cette fonction renvoie une liste avec chaque element doubler mais la liste est inversee  
    //affiche_rec(FctBegaye1(l1)); // Ne marche pas
    return 0;
}
